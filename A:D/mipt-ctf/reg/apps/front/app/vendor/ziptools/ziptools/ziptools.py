...
def extractzipfile(zipname,            # pathname of zipfile to extract from
                   ...
                   maxsize=100 * 1024):# max file size
    """
    -----------------------------------------------------------------------
    ...
    CAVEAT - DATES: extracts here do not adjust dates of zipped items 
    for the local timezone and DST policy; you may or may not need to care.
    See http://learning-python.com/post-release-updates.html#unziptimes.
    -----------------------------------------------------------------------
    """
    ...
    totalsize = sum(e.file_size for e in zipfile.infolist())
    if totalsize > maxsize:
        raise Exception("Extracted size too big")
